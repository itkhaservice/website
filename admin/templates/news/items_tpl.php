        ...
        <table class="table table-hover table-striped mb-0 text-sm">
            <thead class="bg-light">
                <tr>
                    ...
                    <?php } ?>
                    <th>Tiêu đề</th>
                    <?php if($com=='staff') echo '<th>Chức vụ</th>'; ?>
                    <?php if($com=='feedback') echo '<th>Công ty / Vị trí</th>'; ?>
            ...
            <tbody id="sortable-list" data-table="<?=$table_db?>">
                ...
                <tr data-id="<?=$v['id']?>" class="ui-sortable-handle">
                    ...
                    <td class="text-center">
                        <input type="number" class="form-control form-control-sm text-center update-stt text-dark font-weight-bold" value="<?=($v['stt']!='')?$v['stt']:0?>" data-id="<?=$v['id']?>" data-table="<?=$table_db?>" style="width: 70px; margin: auto;">
                    </td>
                    <?php if($com != 'giatri' && $type != 'gioi-thieu') { ?>
                    <td class="text-center">
                        <?php 
                            $img_src = ($v['photo'] != '') ? '../'.$v['photo'] : 'https://placehold.co/60x40?text=No+Image';
                            $img_size = "width='60' height='40'";
                            if($com == 'themanh') $img_size = "width='56' height='56'";
                        ?>
                        <img src="<?=$img_src?>" class="rounded shadow-sm border" <?=$img_size?> style="object-fit:cover;">
                    </td>
                    <?php } ?>
                    <td>
                        <span class="font-weight-bold text-dark"><?=$v['ten_vi']?></span>
                    </td>
                    <?php if(isset($v['chucvu'])){ ?>
                        <td><small class="text-muted"><?=$v['chucvu']?></small></td>
                    <?php } ?>
                    <td class="text-center">
                ...
                <?php }} else { ?>
                <tr><td colspan="9" class="text-center py-4 text-muted">Chưa có dữ liệu.</td></tr>
                <?php } ?>
            </tbody>
        </table>
    </div>
